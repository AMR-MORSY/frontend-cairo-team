<template>
  ...
  <div class="w-screen-2xl px-5 mt-20 py-10">
    <Card class="max-w-sm mx-auto">
      <template #content>
        <p
          class="text-center text-font-main-color font-Signika font-extrabold text-lg"
        >
          Log in
        </p>
        ...
        <form @submit.prevent="submitLoginForm" novalidate>
          ...
          <div class="flex flex-col justify-center py-2 mt-5">
            ...
            <button class="block text-white rounded-xl py-2 bg-font-main-color">
              Login
            </button>
            ...
            <div class="mt-5 flex justify-between items-center">
              <router-link
                ...
                class="text-xs md:text-base font-bold font-Signika text-font-main-color underline underline-offset-1"
      ...
      User.login(form)
        .then((response) => {
          ...
          if (response.data.message == "User loged in successfully") {
            ...
            this.$store.dispatch("userNotifications", response.data.user_data.notifications);/////////notifications here is count 
            const redirectPath = this.$route.query.redirect;///////////////////redirect user to his required path incase it was requested before authentication
